...
void STM32_SPI3_Init(void)
{
	...
	MFRC522_SCK = 0; // 工作在模式0 时钟线在空闲的时候是低电平
	MFRC522_CS(1);	 // 片选引脚置高电平  																			//启动SP3
}
...
uint8_t SPI3_Send(uint8_t txd)
{
	...
	for (i = 7; i >= 0; i--)
	{
		...
		if (txd & (1 << i))
		{
			MFRC522_MOSI = 1; // MOSI设置为高电平
		}
		else
		{
			MFRC522_MOSI = 0; // MOSI设置为低电平
		}

		MFRC522_SCK = 0; // 设置时钟线为低电平，MOSI引脚输出数据
		delay_us(1);

		MFRC522_SCK = 1; // 输出完数据后同时进行数据的接收（高位先收），就要拉高时钟线
		delay_us(1);
		if (MFRC522_MISO)
		{
			recv_data |= (1 << i);
